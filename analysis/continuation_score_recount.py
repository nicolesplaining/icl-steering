"""Recount continuation development scores without importing the production scorer."""
...
def recount(run):
    ...
    assert len(questions) == 128 and len(rows) == 1664
    ...
    for metric in ['primary', 'audited']:
        ...
        for rank, name in enumerate(sorted(p_values, key=p_values.get)):
            running = max(running, min(1., (12-rank)*p_values[name]))
    ...
    checks = {'icl_screen': True, 'zero_gain': (counts['steered']-counts['zero'])/128 >= .03,
              'beats_controls': all(counts['steered'] > counts[k] for k in
                                    ['mean', 'scalar', 'position_mean', 'position_scalar', 'permuted']),
    ...
    return {'status': 'passed', 'checked_at': datetime.now(timezone.utc).isoformat(),
            ...
            'rows': len(rows), 'conditions': len(groups), 'paired_contrasts': 24,
